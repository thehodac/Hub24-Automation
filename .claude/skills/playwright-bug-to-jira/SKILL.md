...
description: >-
  Maintain or extend the automated bug-report + Jira workflow for this
  Playwright + TypeScript project. Use when failing tests should
  ...
  "tạo ticket jira khi test fail", "chuyển ticket sang Done khi pass".
---

# Playwright → Bug Report → Jira workflow

This describes the custom Playwright reporter that turns test failures into bug
reports and Jira tickets, and updates them when tests pass again.

> **Already installed in this repo.** `reporters/bug-report.ts`, `reporters/jira.ts`
> and `utils/apiEvidence.ts` exist and are registered in `playwright.config.ts`.
> Use this skill to **change or extend** that behaviour — read the existing files
> first and edit them. Only build from scratch when setting this up in a new repo.
...
## Constraints / gotchas
- `JIRA_CREATE_TICKETS=true` creates **real tickets in the real Jira project**.
  Claude Code runs on the developer's own machine, so a local test run with the
  switch on will file real bugs. Keep it `false` unless deliberately enabling it,
  and confirm with the user before turning it on.
