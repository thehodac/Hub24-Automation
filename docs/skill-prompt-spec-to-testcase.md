...
description: >-
  ...
  Trigger on: "generate test cases from spec", "generate test cases from functional spec",
...
## Running it
- Run on demand, whenever the Functional Specs change. Specs don't change daily,
  so there is no scheduled run.
- Requires the Atlassian (Confluence) connector to be connected.
